...
class MortgageMixin(object):
    ...
    def __init__(self, term, rate, face, ratedict = None):
        """
        ...
        """
        if ratedict:
            super(MortgageMixin, self).__init__(term, rate, face, ratedict)
        else:
            super(MortgageMixin, self).__init__(term, rate, face)

        """
        I made the if statement because the Variable Mortgage and Fixed Mortgage need to call the mixin __init__
        first to ensure the correct monthly payment functions are used.  However, the VariableLoan and FixedLoan 
        have different parameters, one being rateDict.   I could also put a dummy ratedict parameter on the fixedloan, 
